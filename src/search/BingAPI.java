...
public class BingAPI {
	...
	public ArrayList<Result> find_Bing_Results(String main_query) {
		...
		try {
			...
			String searchURL = "https://api.datamarket.azure.com/Bing/Search/Web?Query='"
					+ URLEncoder.encode(query, charset)
					+ "'&$format=JSON&$top=30";
			URL url = new URL(searchURL);
			HttpURLConnection urlConn = (HttpURLConnection) url
					.openConnection();
			String basic_auth = new String(Base64.encodeBase64((accountKey
					+ ":" + accountKey).getBytes()));
			...
			System.err.println("Failed to retrieve results from Bing :"
					+ exc.getMessage());
		}
		return BingResults;
	}
