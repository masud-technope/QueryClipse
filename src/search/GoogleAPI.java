...
public class GoogleAPI {
	...
	private String get_the_search_results(int startIndex, String main_query) {
		...
		try {
			...
			// String
			// String
			// google="https://www.googleapis.com/customsearch/v1?key="+api_key+"&cx=017576662512468239146:omuauf_lfve&q=";
	...
	private String get_the_search_results1(int startIndex, String main_query) {
		// code for getting results
		String responseBody = "";
		try {
			...
			String google = "https://www.googleapis.com/customsearch/v1?key="
					+ api_key + "&num=10&start=" + startIndex + "&cx="
					+ search_engine_id + "&q=";
			String charset = "UTF-8";
	...
	public ArrayList<Result> find_Google_Results(String main_query) {
		...
		try {
			for (int start = 1; start < 15; start += 10) {
				...
				ArrayList<Result> tempList = gooJson
						.parse_json_code2(responseBody);
				// now add them to the main list
				for (int k = 0; k < tempList.size(); k++) {
					Google_Results.add((Result) tempList.get(k));
				}
				if (tempList.size() < 10)
					break; // if results less than page size
			}
	...
	public static void main(String args[]) {
		...
		try {
			...
			// String searchQuery
			// ="java.io.InterruptedIOException: SocketOutputStream";
